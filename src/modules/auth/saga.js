...
function* signInWithEmailSaga({username, password}) {
  try {
    ...
    if (errors.size) {
      ...
      const {token, user} = yield call(loginWithEmail, {username, password});
      yield call(doLoginSuccess, token, user);
    }

    // yield put({ type: Actions.SIGN_IN_WITH_EMAIL_SUCCESS, payload: { token, user: { ...user, id: 2 } } });
    // yield call(navigationService.navigate, screens.root.Home);
    // yield call(AsyncStorage.setItem, 'token', token);
  } catch (e) {
