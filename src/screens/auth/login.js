...
import {Header, Divider, Text, ThemedView, Loading} from 'src/components';
...
class LoginScreen extends React.Component {
  ...
  render() {
    const {
      navigation,
      auth: {pending, pendingFacebook, pendingGoogle, loginError},
      requiredLogin,
      screenProps: {t, theme},
    } = this.props;
    const {username, password} = this.state;
    ...
    return (
      <ThemedView isFullView>
        <Loading visible={pendingGoogle || pendingFacebook} />
        <Header
          leftComponent={
            !requiredLogin && (
              <IconHeader
        ...
        <KeyboardAvoidingView behavior="height" style={styles.keyboard}>
          <ScrollView>
            <Container>
              ...
              <SocicalAccounts style={styles.viewSocial} />
            </Container>
          </ScrollView>
