...
class AttributeVariable extends React.Component {
  onSelectOption = (option) => {
    ...
    onSelectAttribute(attribute.get('id'), attribute.get('name'), option.get('option'), option.get('name'));
  };
  render() {
    ...
    return (
      <>
        <Text>
          {attribute.get('name')}: <Text colorSecondary>{attributeSelected ? attributeSelected.get('option_name') : ''}</Text>
        </Text>
        <ScrollView
